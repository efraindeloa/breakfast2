...
const OrderDetailScreen: React.FC = () => {
  ...
  const statusHistory: StatusHistory[] = useMemo(() => {
    ...
    const statusLabels: Record<OrderStatus, string> = {
      'orden_enviada': t('orderDetail.orderSent'),
      'orden_recibida': t('orderDetail.orderReceived'),
      'en_preparacion': t('orderDetail.inPreparation'),
      'lista_para_entregar': t('orderDetail.readyToDeliver'),
      'en_entrega': t('orderDetail.inDelivery'),
      'entregada': t('orderDetail.delivered'),
      'con_incidencias': t('orderDetail.withIssues'),
      'orden_cerrada': t('orderDetail.orderClosed'),
      'cancelada': t('orderDetail.cancelled'),
    };
    ...
    return (
      <div className="pb-32 overflow-y-auto bg-background-light dark:bg-background-dark min-h-screen">
        ...
        <div className="px-4 py-6 text-center">
          <p className="text-gray-500 dark:text-gray-400">{t('orderDetail.noActiveOrder')}</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-blue-900 dark:text-blue-200 font-medium mb-1">
                  {t('orderDetail.modificationsTitle')}
                </p>
                <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                  {t('orderDetail.modificationsMessage')}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm text-amber-900 dark:text-amber-200 font-medium mb-1">
                  {t('orderDetail.orderInPreparation')}
                </p>
                <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                  {t('orderDetail.orderInPreparationMessage')}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mb-6 bg-white dark:bg-[#2d2516] rounded-xl p-4 shadow-sm border border-gray-100 dark:border-[#3d3321]">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-400">{t('orderDetail.table')}:</span>
              <span className="text-sm font-semibold text-[#181411] dark:text-white">#04</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-400">{t('orderDetail.sendDate')}:</span>
              <span className="text-sm font-semibold text-[#181411] dark:text-white">{timestamp.date}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600 dark:text-gray-400">{t('orderDetail.sendTime')}:</span>
              <span className="text-sm font-semibold text-[#181411] dark:text-white">{timestamp.time}</span>
            </div>
            {isGroupOrder && (
              <div className="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-800">
                <span className="text-sm text-gray-600 dark:text-gray-400">{t('orderDetail.type')}:</span>
                <span className="text-sm font-semibold text-primary">{t('orderDetail.groupOrder')}</span>
              </div>
            )}
          </div>
        </div>
          ...
          {orderItems.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              <div className="pt-3 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
                <span className="text-base font-bold text-[#181411] dark:text-white">{t('orderDetail.total')}:</span>
